            ...
            {bookings.map((booking, index) => (
              <tr key={index} className='border-t border-borderColor text-gray-500'>
                ...
                <td className='p-3 flex items-center gap-3'>
                  ...
                  <p className='fontmedium max-md:hidden'>{booking.car.brand} {booking.car.model}</p>
                </td>
                ...
                <td className='p-3'>
                  {booking.status === 'pending' ? (
                    ...
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${booking.status ===
                      'confirmed' ? 'bg-green-100 text-green-500': 'bg-red-100 text-red-500'}`}>
                      {booking.status}
                    </span>
                  )}
                </td>
